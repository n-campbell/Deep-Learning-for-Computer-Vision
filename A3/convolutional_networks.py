...
"""
from os import device_encoding, devnull
          ...
          for f in range(F):
            for h_out in range(H_out):
              for w_out in range(W_out):
                # split up n-th picture into filter-sized pieces
                x_small = x_pad[n, :, h_out*stride:h_out*stride+HH, w_out*stride:w_out*stride+WW]
                out[n, f, h_out, w_out] = torch.sum(x_small * w[f]) + b[f]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        # take the max along each color channel of each window of each image 
        for n in range(N):
            for h_out in range(H_out):
              for w_out in range(W_out):
                x_small = x[n, :, h_out*stride:h_out*stride+pool_height, w_out*stride:w_out*stride+pool_width]
                out[n, :, h_out, w_out], indices = torch.max(x_small.reshape(C,-1), dim=1)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
          for h_out in range(H_out):
            for w_out in range(W_out):
              x_small = x[n, :, h_out*stride:h_out*stride+pool_height, w_out*stride:w_out*stride+pool_width]
              dw = torch.zeros_like(x_small.reshape(C ,-1))
              max_vals, max_indices = torch.max(x_small.reshape(C,-1),-1)
              dw[torch.arange(C), max_indices] = dout[n, :, h_out, w_out]
              dx[n,:,h_out*stride:h_out*stride+pool_height,w_out*stride:w_out*stride+pool_width] += dw.reshape(x_small.shape)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        H_conv = int((H + 2 * conv_param['pad'] - filter_size) / conv_param['stride'] + 1)
        W_conv = int((W + 2 * conv_param['pad'] - filter_size) / conv_param['stride'] + 1)
        H_out = int((H_conv - pool_param['pool_height']) / pool_param['stride'] + 1)
        W_out = int((W_conv - pool_param['pool_width']) / pool_param['stride'] + 1)
    ...
    def loss(self, X, y=None):
        ...
        scores, cache1 = Conv_ReLU_Pool.forward(X, self.params['W1'], self.params['b1'], conv_param, pool_param)
        scores, cache2 = Linear_ReLU.forward(scores,self.params['W2'], self.params['b2'])
        scores, cache3 = Linear.forward(scores, self.params['W3'], self.params['b3'])
        ...
        dout, dw, db = Linear.backward(dout, cache3)
        loss += torch.sum(self.params['W3'] ** 2) * self.reg
        grads['W3'] = dw + 2 * self.reg * self.params['W3']
        grads['b3'] = db

        dout, dw, db = Linear_ReLU.backward(dout, cache2)
        loss += torch.sum(self.params['W2'] ** 2) * self.reg
        grads['W2'] = dw + 2 * self.reg * self.params['W2']
        grads['b2'] = db

        dout, dw, db = Conv_ReLU_Pool.backward(dout,cache1)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i,num_filter in enumerate(num_filters):
          ...
          if self.batchnorm:
            self.params['gamma' + str(i+1)] = torch.ones(num_filter, device=device, dtype = dtype)
            self.params['beta' + str(i+1)] = torch.zeros(num_filter, device = device, dtype = dtype)

          H_out = int((H_out + 2 * conv_param['pad'] - filter_size) / conv_param['stride'] + 1)
          W_out = int((W_out + 2 * conv_param['pad'] - filter_size) / conv_param['stride'] + 1)
          
          if i in max_pools:
            H_out = int((H_out - pool_param['pool_height']) / pool_param['stride'] + 1)
            W_out = int((W_out - pool_param['pool_width']) / pool_param['stride'] + 1)

          if weight_scale == 'kaiming':
            self.params['W' + str(i+1)] = kaiming_initializer(C, num_filter, K = filter_size, relu = True, dtype = dtype, device = device)
          else:
            self.params['W' + str(i+1)] = weight_scale * torch.randn(num_filter, C, filter_size, filter_size, dtype = dtype, device = device)
          self.params['b' + str(i+1)] = torch.zeros(num_filter, dtype=dtype,device= device)
          
          C = num_filter

        # initialize final linear layer
        if weight_scale == 'kaiming':
          self.params['W' + str(self.num_layers)] = kaiming_initializer(num_filter * H_out * W_out, num_classes,K = None, relu = False, dtype = dtype, device = device)
        else:
          self.params['W' + str(self.num_layers)] = weight_scale * torch.randn(num_filter * H_out * W_out, num_classes, dtype = dtype, device = device )
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          if i in self.max_pools:
            if self.batchnorm:
              scores, cache_dict['BN' + str(i+1)] = Conv_BatchNorm_ReLU_Pool.forward(scores, 
              ...
              scores, cache_dict['CRP' + str(i+1)] = Conv_ReLU_Pool.forward(scores, 
                                                      self.params['W' + str(i+1)], 
                                                      self.params['b' + str(i+1)], 
                                                      conv_param, pool_param)
          else:
            if self.batchnorm:
              scores, cache_dict['BN' + str(i+1)] = Conv_BatchNorm_ReLU.forward(scores, 
                                                    self.params['W' + str(i+1)], 
                                                    self.params['b' + str(i+1)],
                                                    self.params['gamma' + str(i+1)], 
                                                    self.params['beta' + str(i+1)],
              ...
              scores, cache_dict['CR' + str(i+1)] = Conv_ReLU.forward(scores, 
                                                    self.params['W' + str(i+1)], 
                                                    self.params['b' + str(i+1)], 
                                                    conv_param)
        
        scores, cache_dict['output'] = Linear.forward(scores,
        ...
        loss += torch.sum(self.params['W' + str(self.num_layers)] ** 2) * self.reg
        dout, dw, db = Linear.backward(dout, cache_dict['output'])
        grads['W' + str(self.num_layers)] = dw + 2 * self.params['W' + str(self.num_layers)] * self.reg
        grads['b' + str(self.num_layers)] = db

        for i in reversed(range(self.num_layers-1)):
          if self.batchnorm:
            if i <= (self.num_layers-2):
              loss += torch.sum(self.params['gamma' + str(i+1)] ** 2) * self.reg
          loss += torch.sum(self.params['W' + str(i+1)] ** 2) * self.reg
      
          if i in self.max_pools:
            if self.batchnorm:
              dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout,cache_dict['BN'+str(i+1)])
              grads['W' + str(i+1)] = dw + 2 * self.reg * self.params['W' + str(i+1)]
              grads['b' + str(i+1)] = db
              grads['gamma' + str(i+1)] = dgamma + 2 * self.reg * self.params['gamma' + str(i+1)]
              grads['beta' + str(i+1)] = dbeta
            else:
              dout, dw, db = Conv_ReLU_Pool.backward(dout, cache_dict['CRP' + str(i+1)])
              grads['W' + str(i+1)] = dw + 2 * self.reg * self.params['W' + str(i+1)]
              grads['b' + str(i+1)] = db
          else:
            if self.batchnorm:
              dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, cache_dict['BN' + str(i+1)])
              grads['W'+str(i+1)] = dw + 2 * self.reg * self.params['W'+str(i+1)]
              grads['b' + str(i+1)] = db
              grads['gamma' + str(i+1)] = dgamma + 2 * self.reg * self.params['gamma' + str(i+1)]
              grads['beta' + str(i+1)] = dbeta
            else:
              dout, dw, db = Conv_ReLU.backward(dout, cache_dict['CR' + str(i+1)])
              grads['W' + str(i+1)] = dw + 2 * self.reg * self.params['W' + str(i+1)]
              grads['b' + str(i+1)] = db
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims = data_dict['X_train'].shape[1:], 
                        num_classes = 10, 
                        num_filters = [64,64,128], max_pools = [0,1,2],
                        weight_scale = weight_scale, reg = 1e-5,
                         dtype = dtype, device = device)
    solver = Solver(model, data_dict, num_epochs = 300, batch_size = 128,
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            var = 1./N * torch.sum((x_centered)**2, dim=0)
            ...
            cache = (x_hat, gamma, x_centered, var, std, std_inv, eps, bn_param)
            ...
            normed = (x - running_mean) / torch.sqrt(running_var  + eps)
    ...
    def backward(dout, cache):
        ...
        x_hat, gamma, x_centered, var, std, std_inv, eps, bn_param = cache
        N,D = dout.shape

        if bn_param['mode'] == 'train':
          dbeta = torch.sum(dout, axis=0)
          dgammax = dout 
          dgamma = torch.sum(dgammax*x_hat, axis=0)
          dxhat = dgammax * gamma
          divar = torch.sum(dxhat*x_centered, axis=0)
          dxmu1 = dxhat * std_inv
    ...
    def backward_alt(dout, cache):
        ...
        dbeta = torch.sum(dout, dim=0)
        dgamma = torch.sum(x_hat * dout, dim=0)
        dx = (gamma * std_inv / N) * (N * dout - x_hat * dgamma - dbeta)
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        out, cache = BatchNorm.forward(x.permute(1,0,2,3).reshape(C,-1).t(), 
                                        gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        N,C,H,W = dout.shape
        out = dout.permute(1,0,2,3).reshape(C,-1).t()
        dx, dgamma, dbeta = BatchNorm.backward(out,cache)
