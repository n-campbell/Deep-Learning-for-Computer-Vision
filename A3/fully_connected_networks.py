...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_input = torch.mm(X,self.params['W1']) + self.params['b1']
        hidden = torch.max(torch.zeros_like(hidden_input), hidden_input)
        scores = torch.mm(hidden,self.params['W2']) + self.params['b2']
        ...
        grads['W2'] = torch.mm(hidden.t(), grad_scores) + 2 * self.reg * self.params['W2']
        grads['b2'] = torch.sum(grad_scores, dim=0)
        
        dh = torch.mm(grad_scores, self.params['W2'].t())
        dh[hidden <=0] = 0
        grads['W1'] = torch.mm(X.t(), dh) + 2 * self.reg * self.params['W1']
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          scores, cache_dict['h' + str(i+1)] = Linear_ReLU.forward(scores, self.params['W' + str(i+1)], self.params['b' + str(i+1)])
          if self.use_dropout:
            scores, cache_dict['drop' + str(i+1)] = Dropout.forward(scores, self.dropout_param)
        scores, cache_dict['output'] = Linear.forward(scores, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        ...
        # interate backward through the layers
        for i in range(self.num_layers-1)[::-1]:
          if self.use_dropout:
            dout = Dropout.backward(dout, cache_dict['drop' + str(i+1)])
          dout, dw, db  = Linear_ReLU.backward(dout, cache_dict['h' + str(i+1)])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, device = device,
                    optim_config = {'learning rate': 1e-3},
...
def get_three_layer_network_params():
    ...
    learning_rate = 7e-2  # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ################################################################
    #                             END OF YOUR CODE                 #
