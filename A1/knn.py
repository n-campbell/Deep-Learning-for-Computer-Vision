...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_test):
        for j in range(num_train):
            dists[j,i] = torch.sum(((x_test[i].sub(x_train[j])).square()))
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_test):
        ...
        x_test_flat = x_test[i].view(-1)
        dists[:,i] = torch.sum((((x_train_flat).sub(x_test_flat)).square()),1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # dists = sqrt((x-y)^2)
    # (x-y)^2 = x^2 +y^2 -2xy
    test_sum = torch.sum(x_test.view(num_test,-1).square(),1)
    train_sum = torch.sum(x_train.view(num_train,-1).square(),1)
    product = x_train.view(num_train,-1).mm((x_test.view(num_test,-1)).t()) 
    dists =  test_sum + train_sum.view(-1,1) - 2 * product
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        y_pred[i] = torch.mode(k_nearest)[0]
