...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if (stop//10) < 1:
        ...
        nearest_ten = (start + 9) // 10 * 10;
        x = torch.arange(nearest_ten,stop+1,10);
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx_cols = torch.arange(3)
    idx_rows = torch.tensor([1, 0, 3])
    y = x[idx_rows, idx_cols]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    pos_sum = torch.sum(x[x>0]).tolist()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return pos_sum
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    y = x.clone()
    M = y.shape[0]
    mean = (1/M)*torch.sum(y,dim=0)
    std = torch.sqrt((1/(M-1)) * torch.sum(torch.square(y - mean.view(1,-1)),dim=0))
    y = torch.div(torch.sub(y, mean), std)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
