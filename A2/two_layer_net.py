...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_input = torch.mm(X,params['W1']) + params['b1']
...
def nn_predict(
    ...
    scores -= torch.max(scores)
    norm_scores = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1, keepdims = True)
    y_pred = torch.argmax(norm_scores,dim=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = torch.linspace(0.01,1.0,5)
    learning_rate_decays = torch.linspace(0.0001,0.1,3)
    regularization_strengths = torch.linspace(1e-7,0.01,5)
        ...
        for hs in hidden_sizes:
            for rs in regularization_strengths:
                for lrd in learning_rate_decays:
                    ...
                    if stats['val_acc_history'][len(stats['val_acc_history'])] > best_val_acc:
                        best_val_acc = stats['val_acc_history'][len(stats['val_acc_history'])] 
                        best_net = net
                        best_stat = stats
    #############################################################################
    #                               END OF YOUR CODE                            #
