...
def svm_loss_vectorized(
    ...
    # compute prediction scores and correct scores
    scores = torch.mm(X,W) # size = N x C
    correct_scores = scores[torch.arange(X.shape[0]),y].view(-1,1) # size = N
    
    # identify scores that are below the margin
    # the score margin is dtermined column-wise
    margins = torch.max(torch.zeros_like(scores), scores - correct_scores + 1) # size = N x C
    margins[torch.arange(X.shape[0]), y] = 0

    # each row in margin is the loss for a given sample
    # so to get the loss we sum over them and divide by the number of samples, N
    loss = torch.sum(margins) / margins.shape[0]

    # add the L2 regression term
    loss += reg * torch.sum(W * W)
    ...
    # make a binary version of the margins such that 
    # each column is the loss count for a given class
    binary = margins.clone() # size = N x C
    binary[margins > 0] = 1
    row_sum = torch.sum(binary, dim=1) # size = N x 1
    binary[torch.arange(X.shape[0]), y] = -row_sum.view(1,-1)
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    scores = torch.mm(X,W)
    values, indices = torch.max(scores, dim=1) # both size = N x 1
    y_pred = indices
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = torch.linspace(0.0001,0.1,5)
    regularization_strengths = torch.linspace(1e-5,1,5)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    # Replace "pass" statement with your code
    model = cls
    _ = model.train(data_dict['X_train'], data_dict['y_train'], lr, reg, num_iters)
    y_train_pred = model.predict(data_dict['X_train'])
    train_acc = y_train_pred[y_train_pred == data_dict['y_train']].shape[0] / data_dict['X_train'].shape[0]

    y_val_pred = model.predict(data_dict['X_val'])
    val_acc = y_val_pred[y_val_pred == data_dict['y_val']].shape[0] / data_dict['X_val'].shape[0]
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        # numerical stability: shift values in scores so highest value is 0 
...
def softmax_loss_vectorized(
    ...
    # convert to normalied probabilities row wise for each training sample
    norm_scores = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1, keepdims = True)

    # the loss is the negative log of the correct normalized probability
    loss = torch.sum(-torch.log(norm_scores[torch.arange(X.shape[0]),y]))
    loss = loss/X.shape[0] + reg * torch.sum(W*W)

    # calculate the gradient of the loss
    grad_scores = norm_scores.clone()
    grad_scores[torch.arange(X.shape[0]),y] -= 1
    dW = torch.mm(X.t(), grad_scores)
    dW = dW/X.shape[0] + 2 * reg * W
